...
func main() {
	...
	apiHandler, err := api.NewVoter()
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	apiHandler1, err := api.NewPoll()
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}

	apiHandlerVotes, err := api.NewVote()
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	//vl := api.NewVoterApi()
	//app.Post("/voter", vl.AddVoter())
	...
	app.Delete("/todo/:id<uint>", apiHandler.DeleteToDo)
