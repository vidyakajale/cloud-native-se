...
func TestAddHardCodedItem(t *testing.T) {
	...
	//Use GetItem to check if item is added in DB
	dbItem, err := DB.GetItem(item.Id)
	assert.NoError(t, err, "Error getting item from DB")

	assert.Equal(t, item.Id, dbItem.Id, "Id matches with DB ")
	assert.Equal(t, item.IsDone, dbItem.IsDone, "IsDone matches with DB ")
	assert.Equal(t, item.Title, dbItem.Title, "Title matches with DB ")
...
func TestGetItem(t *testing.T) {
	item := db.ToDoItem{
		Id:     4,
		Title:  "Learn Why Professor Mitchell is the BEST! :-)",
		IsDone: false,
	}
	t.Log("Testing Get Item: ", item)
...
func TestGetAllItem(t *testing.T) {
	...
	todoListCount := len(todoList)
	assert.Equal(t, count, todoListCount, "Count mateches ")

}
func TestUpdateHardCodedItem(t *testing.T) {
	item := db.ToDoItem{
		Id:     123,
		Title:  "This is a test case item to update",
		IsDone: false,
	}
	t.Log("Testing update a Hard Coded Item: ", item)
	...
	assert.NoError(t, err, "Error updating item to DB")

	//Use GetItem to check if item is updated in DB
	dbItem, err = DB.GetItem(item.Id)
	assert.NoError(t, err, "Error getting item from DB")
...
func TestDeleteHardCodedItem(t *testing.T) {
	item := db.ToDoItem{
		Id:     222,
		Title:  "This is a test case item to delete",
		IsDone: false,
	}
	t.Log("Testing delete a Hard Coded Item: ", item)
	...
	assert.NoError(t, err, "Error dwlwting item from DB")

	//Use GetItem to check if item is updated in DB
	dbItem, err = DB.GetItem(item.Id)
	assert.NoError(t, err, "Error getting item from DB")

	assert.NotEqual(t, item.Id, dbItem.Id, "Id not available in DB ")
