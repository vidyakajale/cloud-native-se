...
//
// ANSWER: The fields in ToDo struct are lower case that means they cannot be exported. The struct with fields in lower case are only visible to code in the same package, which makes them private.
// Fields with upper case are visible in other packages. That is the reason it is good design decision.
type ToDo struct {
	toDoMap    DbMap
	dbFileName string
}
...
func (t *ToDo) RestoreDB() error {
	...
	fmt.Println("RestoreDB() is implemented")
...
func (t *ToDo) AddItem(item ToDoItem) error {
	...
	for _, toDoItem := range t.toDoMap {

		if t.toDoMap[toDoItem.Id].Id == item.Id {
			return errors.New("Item aleardy exists")
		}
...
func (t *ToDo) DeleteItem(id int) error {
	...
	for _, toDoItem := range t.toDoMap {
		...
		if t.toDoMap[toDoItem.Id].Id == id {
			delete(t.toDoMap, id)
			err = t.saveDB()
			return err
		}
...
func (t *ToDo) GetItem(id int) (ToDoItem, error) {
	...
	//Create an empty slice of ToDoItem
	var toDoItem ToDoItem

	//iterate over our map and add each item to our slice.
	for _, item := range t.toDoMap {

		if t.toDoMap[item.Id].Id == id {
			toDoItem = item
			break
		}
